"""papers URL Configuration

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/3.0/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.urls import path,re_path
from . import views

urlpatterns = [
    path('',views.dashboard ,name="Teacher_dashboard"),
    path('Exams/',views.exams ,name="Teacher_exams"),
    path('Papers/',views.Forms ,name="Teacher_Forms"),
    path('Course/',views.courses ,name="Teacher_courses"),
    path('Results/',views.results ,name="Teacher_results"),
    path('Students/',views.students ,name="Teacher_students"),
    path('Signin/',views.Signin ,name="Teacher_Signin"),
    path('api/',views.api ,name="api"),
    re_path(r'.+',views.dashboard)
]
